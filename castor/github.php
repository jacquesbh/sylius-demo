...
function initGithubEnv(
    #[AsOption(description: 'Environment', suggestedValues: ['prod', 'staging'])] ?string $env = null,
    #[AsOption(description: 'Deployment branch', suggestedValues: ['master'])] ?string $branch = null,
): void {
    ...
    $environment = $env ?? io()->ask('Which environment?', 'staging');
    $command = <<<CMD
    curl --silent -L \
    -X PUT \
    ...
    $branch = $branch ?? io()->ask('Deployment branch?', 'master');
    run(sprintf(
